"""
Mean Pooling: LSTM unidirectional + masked mean pooling.
WavLM-Large (frozen) -> LSTM -> Mean Pooling -> Classifier.
"""
...
def _load_presets_from_json() -> dict:
    ...
    for preset_name, variants in data["presets"].items():
        if "mean" in variants:
            presets[preset_name] = variants["mean"]
    return presets
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="LSTM + Mean Pooling (Masked)")
    parser.add_argument(
...
class LSTMMeanModel(nn.Module):
    """WavLM-Large (frozen) -> LSTM unidirectional -> Masked Mean Pooling -> Classifier."""
    ...
    def forward(self, x: torch.Tensor, attention_mask: torch.Tensor | None = None) -> torch.Tensor:
        ...
        if attention_mask is not None:
            ...
            valid_mask = feature_mask.unsqueeze(-1).float()
            c_context = (H_lstm * valid_mask).sum(dim=1) / valid_mask.sum(dim=1).clamp(min=1)
        else:
            c_context = torch.mean(H_lstm, dim=1)
...
def main() -> None:
    ...
    print("=" * 80)
    print("PELATIHAN LSTM + MEAN POOLING")
    print("=" * 80)
    print(f"Preset: {args.preset}")
    print(f"Dataset: {config['dataset_name']}")
    print(f"Output dir: {config['output_dir']}")
    print("WavLM-Large: frozen")
    print("Arsitektur: LSTM unidirectional + mean pooling")
    print("=" * 80)
    ...
    print("=" * 80)
    print(f"Model: LSTM unidirectional + mean pooling")
